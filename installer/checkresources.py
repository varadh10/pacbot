...
def _check_baserole(accessKey,secretKey,region,resource,terraform):
    ...
    except iam.exceptions.NoSuchEntityException as ne3: pass
    try:
        response_r4=iam.get_role(RoleName=jsonRead._get_pacrunbatch_role_name())
    except iam.exceptions.NoSuchEntityException as ne4: pass
...
def _detach_policy(iam,rolename,policyarn,roleindex):
    ...
    except iam.exceptions.DeleteConflictException as e:  pass
    except Exception as e: pass
    try:
        iam.delete_role(RoleName=rolename)
    except iam.exceptions.NoSuchEntityException as ne1: pass
    except iam.exceptions.DeleteConflictException as ne2: pass
...
def _check_s3(accessKey,secretKey,region,resource,terraform):
    ...
    if s3.Bucket(bucket_name).creation_date:
        if _check_in_terraform(bucket_name,terraform): return True
        else:
            try:
...
def _check_redshift(accessKey,secretKey,region,resource,terraform):
    ...
    try:
        ...
        if _check_in_terraform(response['Clusters'][0]['Endpoint']['Address'],terraform) == True:
            return True
        else:
            while True:
                try:
...
def _check_batch(accessKey,secretKey,region,resource,terraform):
    ...
    is_deleted=False
    try:
        if _check_in_terraform(response_batch['computeEnvironments'][0]['computeEnvironmentName'],terraform) == True:
            if _check_in_terraform(response_jd['jobDefinitions'][0]['jobDefinitionArn'],terraform) == True:
                if _check_in_terraform(response_jq['jobQueues'][0]['jobQueueArn'],terraform) == True:
                    if _check_in_terraform(response_jq['jobQueues'][1]['jobQueueArn'],terraform) == True:
                        return True
        else:
            while True:
                try:
                        ...
                        if len(response['jobDefinitions']) > 0:
                            ...
                            if len(response['computeEnvironments']) > 0:
                                if response['computeEnvironments'][0]['state'] != 'DISABLED':
                                    batch.update_compute_environment(computeEnvironment
                                         =response['computeEnvironments'][0]['computeEnvironmentName'],state='DISABLED')
                                else:
                                    batch.delete_compute_environment(computeEnvironment
                                        =response['computeEnvironments'][0]['computeEnvironmentName'])
                                    is_deleted=True
                            else:   break
...
def _check_lambda_submit(accessKey,secretKey,region,resource,terraform):
    ...
    try:
        ...
        if response != '' and _check_in_terraform(response['Configuration']['FunctionArn'],terraform):
            ...
            to_be_deleted=True
    except lambda_fn.exceptions.ResourceNotFoundException as e: return False
    try:
        if to_be_deleted:
    ...
    except Exception as e: _remove(resource,True)
    return False
